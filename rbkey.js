...
module.exports = function RBKeyboard() {
  ...
  var processData = function(data) {
    ...
    var touch = data[15];
    if (touchData != touch) {
      touch != 0 ? this.onTouchOn(touch) : this.onTouchOff();
      touchData = touch;
    }
    
    var touchButton = data[13] != 0;
    if (touchButtonData != touchButton) {
      this.onTouchButton(touchButton);
    ...
    for (var i = 0; i < 25; i++) {
      ...
      if ((noteData[i] != 0) != noteOn) {
        ...
        noteOn ? this.onNoteOn(i, velocity) : this.onNoteOff(i, noteData[i]);
        noteData[i] = velocity;
      }
    }
  };
